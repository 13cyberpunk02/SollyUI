...
- `SollyTestContext` — base class for all component tests. Registers the library services and runs bUnit's JSInterop in **Loose** mode, so components that call into `solly.js` (portals, focus traps, theme/palette) render without each JS call being stubbed.
